...
이 레포는 GitHub template repository입니다. 레포 페이지 상단의 **"Use this
template" → "Create a new repository"** 버튼으로 새 프로젝트 레포를 만드는 것이
가장 간단합니다. (템플릿은 fork와 달리 커밋 히스토리 없이 깨끗한 새 레포로
시작됩니다.) GitHub를 쓰지 않으면 레포 내용을 새 프로젝트 폴더로 복사해도
됩니다.

새 프로젝트를 만든 뒤 아래 순서대로 placeholder를 실제 값으로 바꿉니다. 앞
단계가 비어 있으면 Codex가 MVP 범위, 검증 명령, phase 경계를 잘못 판단할 수
있으므로 순서를 건너뛰지 않습니다.
...
템플릿 체크아웃을 받아 두면 `scripts/upgrade.py`로 이 과정을 자동화할 수
있습니다.

```bash
python scripts/upgrade.py --from <template-checkout> --dry-run  # 변경 사항 미리보기
python scripts/upgrade.py --from <template-checkout>            # 적용 + templateVersion 갱신
```
