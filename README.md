...
| 6 | `docs/COMMANDS.md`, `.codex/project-profile.json` | 확정된 기술 스택 기준의 `dev`, `lint`, `test`, `build`, `profile`, `sourceRoots`, `testRoots` | 최소 `test`와 `build`가 비어 있지 않거나 manifest로 감지 가능 |
...
```bash
python scripts/autopilot.py {phase-name} --base main --max-review-fixes 2
```

5. autopilot은 다음 pending step을 `codex/{phase}-step{N}-{name}` 브랜치에서 실행하고 Draft PR을 만듭니다.
6. `python scripts/checks.py --stage manual`, `git diff --check`, Codex read-only review가 통과하면 PR을 ready로 전환하고 squash merge합니다.
7. 리뷰가 실패하면 PR 코멘트, GitHub Issue, `issues/{phase}/issue-N.md`를 남기고 같은 PR 브랜치에서 자동 수정과 재리뷰를 진행합니다.
8. 재시도 후에도 실패하면 PR과 Issue를 열어둔 채 중단합니다.
...
않으면 manual check와 autopilot gate가 실패합니다.
