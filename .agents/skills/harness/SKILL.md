...
8. 문서 간 API, 범위, 실행 방법 충돌이 발견되면 구현 단계에 섞지 말고 별도 문서 동기화 step으로 분리한다.
9. phase별 문서 정합성 규칙은 `phases/{작업명}/docs-checks.json`에 함께 작성한다. MVP나 phase 범위가 바뀌면 `scripts/checks.py`가 아니라 이 파일을 갱신한다.
10. `docs-checks.json`은 정성 규칙의 SSOT가 아니다. final stage에서 기계적으로 잡을 수 있는 핵심 회귀 신호만 담는다.
